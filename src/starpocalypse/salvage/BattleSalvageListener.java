...
public class BattleSalvageListener extends BaseCampaignEventListener {
    ...
    public void reportBattleFinished(CampaignFleetAPI primaryWinner, BattleAPI battle) {
        for (FleetMemberAPI ship : Global.getSector().getPlayerFleet().getMembersWithFightersCopy()) {
            ...
            Global
                ...
                .getStats()
                .getDynamic()
                .getMod("ship_recovery_mod")
                .unmodifyFlat("starpocalype_ship_recovery_fix");
        }
    }
    ...
    public void reportPlayerEngagement(EngagementResultAPI result) {
        if (
            result.didPlayerWin() && ConfigHelper.isStingyRecoveriesCombat()
        ) { // In theory just not adding the listener should be enough, but in my testing it was active anyway sometimes. So better make sure
        ...
        if (ConfigHelper.isStingyNerfHullRestoration()) {
            ...
            for (String flatKey : Global
                ...
                if (
                    !Global
                        ...
                        .getDynamic()
                        .getMod("ship_recovery_mod")
                        .getFlatBonus(flatKey)
                        .getSource()
                        .equals("starpocalype_ship_recovery_fix")
                ) flatMod +=
                    Global
                        .getSector()
                        .getPlayerFleet()
            ...
            ConfigHelper.overwriteOriginalVanillaFloat(
                "baseOwnShipRecoveryChance",
                flatMod + ConfigHelper.getOriginalVanillaFloat("baseOwnShipRecoveryChance")
            );
            ...
            Global
                ...
                .getDynamic()
                .getMod("ship_recovery_mod")
                .modifyFlatAlways("starpocalype_ship_recovery_fix", -flatMod, "Starpocalypse ship recovery fix");
            ConfigHelper.overwriteOriginalVanillaFloat(
                "baseOwnShipRecoveryChance",
                flatMod + ConfigHelper.getOriginalVanillaFloat("baseOwnShipRecoveryChance")
            );
        ...
        for (FleetMemberAPI ship : members) {
            ...
            if (!ship.isAlly() || ConfigHelper.isStingyRecoveriesIncludePlayerShips()) {
                if (ship.isFrigate() && (!isPlayer || ConfigHelper.getStingyRecoveriesCombatPlayerShipsSize() > 1)) {
                    ...
                    ship.isDestroyer() && (!isPlayer || ConfigHelper.getStingyRecoveriesCombatPlayerShipsSize() > 2)
                ) {
                    applyMod = true;
                } else if (
                    ship.isCruiser() && (!isPlayer || ConfigHelper.getStingyRecoveriesCombatPlayerShipsSize() > 3)
                ) {
                    applyMod = true;
            ...
            if (
                !ship.getVariant().hasTag("starpocalypseAppliedStingyWeapon") &&
                !ship.isFighterWing() &&
                (!isPlayer || ConfigHelper.isStingyRecoveriesIncludePlayerShips())
            ) {
