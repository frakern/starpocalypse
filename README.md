...
6. Baseline salvage chance is reduced dependent on hull size. No ships are removed from being salvageable, but require story points.
...
1. Ignore player owned markets altogether (do nothing). This also includes autonomous colonies from Nexerelin.
...
4. Pirates optionally get megaports and heavy batteries.
5. And make sure that the above are met at all times (via a transient listener).
...
5. Stealing a colony item instantly sets your reputation to -100 (hostile).
...
1. Remove access to the Nexerelin high-end seller.
2. Remove blueprint packages from loot tables. You will have to collect blueprints one by one.
3. Reduce chance to recover your own fleet's ships without using story points.
